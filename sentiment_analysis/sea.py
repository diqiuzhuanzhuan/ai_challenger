...
class Data(object):
    ...
    def __truncate_sentence(self, x):
    ...
    def _load_train_and_validation_data(self):
        ...
        train_dataset = tf.data.Dataset.from_generator(self._gen_train_data, (tf.int32, tf.int32, tf.int32), ([None], [None], [self._labels_num]))
        train_dataset = train_dataset.map(lambda *x: (self.__truncate_sentence(x[0]), x[1], x[2])).padded_batch(self._batch_size, padded_shapes=([self._max_length], [None], [None]),
                                                                                                                padding_values=(
                                                                                                                tf.constant(1, dtype=tf.int32), tf.constant(0, dtype=tf.int32), tf.constant(0, dtype=tf.int32)))

        train_dataset = train_dataset.map(lambda *x: (x[0], x[1], tf.one_hot(indices=x[2], depth=4, dtype=tf.int32)))
        ...
        validation_dataset = validation_dataset.map(lambda *x: (x[0], x[1], tf.one_hot(indices=x[2], depth=4, dtype=tf.int32)))
    ...
    def _load_test_data(self):
        test_dataset = tf.data.Dataset.from_generator(self._gen_test_data, (tf.int32, tf.int32, tf.int32), ([None], [None], [self._labels_num]))
        test_dataset = test_dataset.map(lambda *x: (self.__truncate_sentence(x[0]), x[1], x[2]))\
            .padded_batch(self._batch_size, padded_shapes=([self._max_length], [None], [None]),
                                                 padding_values=(tf.constant(1, dtype=tf.int32), tf.constant(0, dtype=tf.int32), tf.constant(0, dtype=tf.int32)))
        test_dataset = test_dataset.map(lambda *x: (x[0], x[1], tf.one_hot(indices=x[2], depth=4, dtype=tf.int32)))
