...
flags.DEFINE_integer("train_batch_size", 36, "Total batch size for training.")
...
class SentimentProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        return self._create_samples(set_type="train")
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    if ex_index < 5:
        ...
        tf.logging.info("input_ids: {}".format(input_ids))
        tf.logging.info("input_mask: {}".format(input_mask))
        tf.logging.info("segment_ids: {}".format(segment_ids))
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    hidden_size = hidden_size * 6
    labels = tf.reshape(labels, [-1, 6, 20])
    labels = labels[:, 0, :]
    ...
    output_weights = [tf.get_variable(
        ...
        initializer=tf.truncated_normal_initializer(stddev=0.02)) for i in range(length)]

    output_bias = [tf.get_variable(
        "output_bias"+str(i), [num_labels], initializer=tf.zeros_initializer()) for i in range(length)]
    ...
    with tf.variable_scope("loss"):
        ...
        logits = [tf.matmul(output_layer, output_weights[i], transpose_b=True) for i in range(length)]
        logits = [tf.nn.bias_add(logits[i], output_bias[i]) for i in range(length)]
        ...
        w = [tf.nn.embedding_lookup(weights[i], tf.argmax(one_hot_labels[:, i, :], axis=1), name="embedding_lookup" + str(i)) for i in range(length)]
        _loss_ = [tf.losses.softmax_cross_entropy(onehot_labels=one_hot_labels[:, i, :], logits=logits[i], weights=w[i]) for i in range(length)]
        probabilities = [tf.nn.softmax(logits[i]) for i in range(length)]
        ...
        d = tf.data.Dataset.from_tensor_slices({
            ...
            "segment_ids":
                tf.constant(
                    ...
                    dtype=tf.int32),
            "label_ids":
                tf.constant(all_label_ids, shape=[num_examples, 20], dtype=tf.int32),
        })
...
if __name__ == "__main__":
    ...
    FLAGS.data_dir = "."
    FLAGS.task_name = "stmt"
    FLAGS.vocab_file = "./checkpoint/vocab.txt"
